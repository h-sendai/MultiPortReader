...
static const char* multiportreader_spec[] =
{
    "implementation_id", "MultiPortReader",
    "type_name",         "MultiPortReader",
    "description",       "MultiPortReader component",
    "version",           "1.0",
...
MultiPortReader::MultiPortReader(RTC::Manager* manager)
      ...
      m_debug(false)
{
    ...
    // Set OutPort buffers
    registerOutPort("multiportreader_out", m_OutPort);

    init_command_port();
    init_state_table();
    set_comp_name("MULTIPORTREADER");
}
...
RTC::ReturnCode_t MultiPortReader::onInitialize()
{
    if (m_debug) {
        std::cerr << "MultiPortReader::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t MultiPortReader::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int MultiPortReader::daq_dummy()
{
    return 0;
}

int MultiPortReader::daq_configure()
{
    fprintfwt(stderr, "MultiPortReader::configure\n");
    //std::cerr << "*** MultiPortReader::configure" << std::endl;
...
int MultiPortReader::parse_params(::NVList* list)
{
...
int MultiPortReader::daq_unconfigure()
{
    fprintfwt(stderr, "MultiPortReader::unconfigure\n");
    //std::cerr << "*** MultiPortReader::unconfigure" << std::endl;
    m_module_list.clear();

    return 0;
}

int MultiPortReader::daq_start()
{
    fprintfwt(stderr, "MultiPortReader::start\n");
    //std::cerr << "*** MultiPortReader::start" << std::endl;
...
int MultiPortReader::daq_stop()
{
    fprintfwt(stderr, "MultiPortReader::stop\n");
    //std::cerr << "*** MultiPortReader::stop" << std::endl;
...
int MultiPortReader::daq_pause()
{
    std::cerr << "*** MultiPortReader::pause" << std::endl;

    return 0;
}

int MultiPortReader::daq_resume()
{
    std::cerr << "*** MultiPortReader::resume" << std::endl;

    return 0;
}

int MultiPortReader::set_data(unsigned char *read_buf, unsigned int data_byte_size)
{
...
int MultiPortReader::write_OutPort()
{
...
{
    if (m_debug) {
        std::cerr << "*** MultiPortReader::run" << std::endl;
    }
...
extern "C"
{
    ...
    {
        RTC::Properties profile(multiportreader_spec);
        manager->registerFactory(profile,
                    RTC::Create<MultiPortReader>,
                    RTC::Delete<MultiPortReader>);
    }
};
